...
The LLM Request Interception capability (ModelProxy) provides a **generic, LLM-agnostic** system for intercepting, transforming, and forwarding LLM requests before they reach any target language model. This ensures that all AI interactions within the UbeCode ecosystem adhere to project rules, SAWai methodology requirements, and organizational policies - regardless of which LLM backend is used.

The capability is implemented as **UbeCLI** - an interactive command-line interface that provides a familiar CLI coding assistant experience with automatic context injection and SAWai rule enforcement.
...
- **SAWai Approval Enforcement**: Validates approval status of referenced capabilities/enablers
...
2. SAWai methodology constraints are enforced
...
- **Compliance**: SAWai approval workflows are automatically enforced
...
**Developer** - A software developer working on UbeCode or related projects who needs AI assistance for code generation, refactoring, or analysis while adhering to project standards.
...
1. **Basic Code Generation**:
   ...
   - Prework: Inject CLAUDE.md, check for capability/enabler specs, add SAWai workflow rules
...
3. **Discovery Mode**:
   ...
   - Output: Documentation following SAWai discovery phase rules
...
| MAIN_SWDEV_PLAN.md | Configuration | SAWai methodology rules |
...
| FR-759004 | SAWai Enforcement | System injects MAIN_SWDEV_PLAN.md rules | Ready for Design | High | Approved |
