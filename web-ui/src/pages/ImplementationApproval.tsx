...
export const ImplementationApproval: React.FC = () => {
  ...
  // Load implementation phase items - only on initial load or workspace change
  useEffect(() => {
  ...
  const getPendingItemsCount = () => {
    return implementationItems.filter(i => i.status !== 'approved' && i.status !== 'rejected').length;
  };

  const getApprovableItems = () => {
    return implementationItems.filter(i => i.status !== 'approved' && i.status !== 'rejected');
  };
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          ...
          <div>
            {implementationApproved ? (
              ...
              >
                {phaseActionLoading ? 'Approving...' : 'Approve Implementation Phase'}
              </Button>
            )}
          </div>
        </div>
